...
def print_system_info():
    ...
    # save info here, cuz getting is slow af (like 2 seconds) for some reason, and i want twoo items from it, so this way i avoid waiting an eternity twice
...
def mergesort(input_):
    ...
    def merge(list_a, list_b):
        ...
        #####################################################################################
        # Modification (credit Sofia (https://github.com/BuckbeakS007)):
        if list_a[0] >= list_b[0-1]:
            #output_ += list_b       
            #output_ += list_a
            #return output_
            return list_b + list_a
        elif list_b[0] >= list_a[0-1]:
            #output_ += list_a
            #output_ += list_b
            #return output_
